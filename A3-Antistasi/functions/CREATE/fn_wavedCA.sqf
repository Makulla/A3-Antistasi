...
[_sideTsk,"rebelAttack",[format ["%2 Is attacking from the %1. Intercept them or we may loose a sector. Intel reports they will attack %3, sending %4 waves.",_nameOrigin,_nameENY,_nameDest,_waves],format ["%1 Attack",_nameENY],_mrkOrigin],getMarkerPos _mrkOrigin,false,0,true,"Defend",true] call BIS_fnc_taskCreate;
...
while {(_waves > 0)} do
{
	...
	if (!_SDKShown) then
		...
		["rebelAttack",[format ["%2 Is attacking from the %1. Intercept them or we may loose a sector. We are expecting %3 waves.",_nameOrigin,_nameENY,_waves],format ["%1 Attack",_nameENY],_mrkDestination],getMarkerPos _mrkDestination,"CREATED"] call A3A_fnc_taskUpdate;
		};
